...
impl SaveFile {
    pub fn read_from_file(path: &Path) -> Result<SaveFile, std::io::Error> {
    ...
    pub fn read_from_local_save() -> Result<SaveFile, std::io::Error> {
    ...
    fn read_crew_member(parser: &mut SaveFileParser ) -> Result<CrewMember, Error> {
        let mut unit = CrewMember::default();
        unit.name = parser.read_string()?;
        unit.race = parser.read_string()?;
    ...
    fn read_animation_state(parser: &mut SaveFileParser) -> Result<AnimationState, Error> {
